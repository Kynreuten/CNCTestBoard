...
properties = {
  ...
  retractHeight: 15 // specifies the retract height instead of using G28 for retracts  (0 ~ disabled)
};

var numberOfToolSlots = 9999;
var maximumClearanceHeight;
...
function onOpen() {
  ...
  if (properties.retractHeight != 0) {
    maximumClearanceHeight = properties.retractHeight;
    if (is3D()) {
      var numberOfSections = getNumberOfSections();
      for (var i = 0; i < numberOfSections; ++i) {
        var section = getSection(i);
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    retracted = true;
    if (properties.retractHeight != 0) {
      writeBlock(gMotionModal.format(0), "Z" + xyzFormat.format(maximumClearanceHeight)); // retract
    } else {
